  ...
  <el-dialog v-model="visible" title="扫码支付" width="400px" :close-on-click-modal="false" @close="handleClose">
    <div style="text-align: center">
      ...
      <div v-else-if="status === 'expired'">
        <el-result icon="warning" title="支付超时" sub-title="请重新发起结算" />
      </div>
      <div v-else>
...
function startPolling() {
  ...
  pollTimer = setInterval(async () => {
    try {
      const res: any = await api.get(`/settlements/${props.settlementId}/payment-status`);
      if (res.paymentStatus === 'paid' || res.settlementStatus === 'settled') {
        status.value = 'paid';
        stopPolling();
      ...
      } else if (res.paymentStatus === 'expired') {
